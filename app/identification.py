...
def identify_df_trends2(df, column, segments_count):
    if df is None:
        raise ValueError("df argument is mandatory and needs to be a `pandas.DataFrame`.")
    ...
    for index, value in enumerate(data, 0):
        ...
        if value != limit and len(values) > window:
            if from_trend:
                going_up = value > start_trend_price
                last_thread = {
                    'from': df.index.tolist()[from_trend],
                    'to': df.index.tolist()[index],
                    'type': TREND_UP if going_up else TREND_DOWN,
                    'price': value
                }

                if going_up:
                    uptrends.append(last_thread)
                else:
                    downtrends.append(last_thread)
            from_trend = index
            start_trend_price = value
            values = list()
    if from_trend:
        value = data[-1]
        index = len(data) - 1
    ...
    up_labels = [letter for letter in string.ascii_uppercase[:len(uptrends)]]
    for up_trend, up_label in zip(uptrends, up_labels):
        for index, row in df[up_trend['from']:up_trend['to']].iterrows():
            df.loc[index, 'Up Trend'] = up_label

    down_labels = [letter for letter in string.ascii_uppercase[:len(downtrends)]]
    for down_trend, down_label in zip(downtrends, down_labels):
        for index, row in df[down_trend['from']:down_trend['to']].iterrows():
            df.loc[index, 'Down Trend'] = down_label

    # print(f"result {last_thread}")
    return df, last_thread
